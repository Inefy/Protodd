param([string]$Root='build/pvz-builder-site-20260925')
...
try {
    $arena=Join-Path $rootPath 'candidate'
    ...
    Assert-Hash (Join-Path $repo 'training/pvz_builder_site_screen.py') $plan.screen_source_sha256
    Assert-Hash $PSCommandPath $plan.launcher_sha256
    Assert-Hash (Join-Path $repo 'build/pvz-builder-select-20260925/diagnostic-report.json') $plan.reference_report_sha256
    Assert-Hash (Join-Path $repo 'build/pvz-builder-select-20260925/selection-review.json') $plan.selection_review_sha256
