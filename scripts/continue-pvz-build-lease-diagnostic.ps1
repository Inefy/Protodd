param([string]$Root='build/pvz-build-lease-diagnostic-20260925')
$ErrorActionPreference='Stop'
$repo=Split-Path -Parent $PSScriptRoot
Set-Location -LiteralPath $repo
$rootPath=(Resolve-Path -LiteralPath $Root).Path
$plan=Get-Content -LiteralPath (Join-Path $rootPath 'diagnostic-plan.json') -Raw|ConvertFrom-Json
$python=Join-Path $repo 'build/model-venv/Scripts/python.exe'
$statusPath=Join-Path $rootPath 'diagnostic-status.json'
...
try {
    $arena=Join-Path $rootPath 'arena'
    if(Test-Path -LiteralPath (Join-Path $rootPath 'diagnostic-report.json')) {
        throw 'Diagnostic report exists; do not duplicate games'
    }
    if(Test-Path -LiteralPath (Join-Path $arena 'processes.json')) {
        throw 'Arena already has a process owner; inspect before recovery'
    }
    ...
    Assert-Hash (Join-Path $repo 'training/pvz_build_lease_diagnostic_review.py') $plan.reviewer_sha256
    ...
    if([DateTime]::UtcNow -ge $deadline){throw 'Diagnostic exceeded two-hour deadline'}
    ...
    $health=(& $python -m training.arena inspect $arena|Out-String)|ConvertFrom-Json
    if($LASTEXITCODE -ne 0 -or @($health.structurally_valid).Count -ne $plan.games -or
       @($health.excluded).Count){throw 'Diagnostic failed normal health gate'}
    Stop-OwnedLaunchers $arena
    Publish 'reviewing'
    & $python -m training.pvz_build_lease_diagnostic_review (Join-Path $rootPath 'diagnostic-plan.json') `
        (Join-Path $rootPath 'diagnostic-report.json')
    if($LASTEXITCODE -ne 0){throw 'Diagnostic review failed; preserve artifacts'}
    Publish 'complete'
} catch {
