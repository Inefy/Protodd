param([string]$Root='build/pvz-available-composition-20260926')
...
try {
    ...
    foreach($entry in $plan.repo_sha256.PSObject.Properties) { Assert-Hash (Join-Path $repo $entry.Name) $entry.Value }
    ...
    Publish 'reviewing'
    & $python -m training.available_composition_review `
