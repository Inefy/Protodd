param([string]$Root='build/pvz-builder-select-20260925')
...
try {
    ...
    if(Test-Path -LiteralPath (Join-Path $arena 'processes.json')) {
        throw 'Diagnostic already has a process owner; inspect before recovery'
    }
    ...
    Publish 'reviewing'
    & $python -m training.pvz_build_lease_mixed_review (Join-Path $rootPath 'diagnostic-plan.json') `
