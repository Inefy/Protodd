param([string]$Root='build/pvz-pylon-lease-20260925')
...
try {
    ...
    Assert-Hash (Join-Path $repo 'training/pvz_build_lease_capped_review.py') $plan.reviewer_sha256
    ...
    Publish 'reviewing'
    & $python -m training.pvz_build_lease_capped_review (Join-Path $rootPath 'candidate-plan.json') `
        (Join-Path $rootPath 'candidate-report.json')
    if($LASTEXITCODE -ne 0){throw 'Capped diagnostic review failed; preserve artifacts'}
    Publish 'complete'
} catch {
