"""Review a source-pinned PvZ mobile-screen development comparison."""
...
def review(plan_path, reference_path, candidate_path):
    ...
    result = dict(schema="protodd-pvz-mobile-screen-review-v1",
    ...
    result["screen_more_games"] = (
        sum(army_gains_8400) / n >= 2 and
        sum(probe_gains_8400) / n >= -3 and
        core_confirmed and
        (not comparable_core_delays or
         sum(comparable_core_delays) / len(comparable_core_delays) <= 720) and
