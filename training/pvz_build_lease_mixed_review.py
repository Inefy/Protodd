"""Review capped and normally completed games without inventing timeout wins."""
...
def review(plan_path):
    ...
    for game_id, game in sorted(schedule.items()):
        ...
        for result in pair:
            if result["opponentBot"] not in (game["homeBot"], game["awayBot"]) or \
                ...
                raise ValueError(f"unhealthy or unexpected bounded pair: {game_id}")
        capped = all(not r["won"] and r["finalFrame"] == plan["frame_limit"] + 2
                     for r in pair)
        decisive = (pair[0]["won"] != pair[1]["won"] and
                    abs(pair[0]["finalFrame"] - pair[1]["finalFrame"]) <= 120)
        if not (capped or decisive):
            raise ValueError(f"neither normal decisive nor exact-cap pair: {game_id}")
        ...
        rows.append(dict(game_id=game_id, map=game["map"], home=game["homeBot"],
                         ...
                         frame=own["finalFrame"], capped=capped,
                         decisive_won=own["won"] if decisive else None,
    ...
    return dict(schema="protodd-pvz-build-lease-capped-review-v1",
                ...
                summary=dict(games=len(rows), frame_limit=plan["frame_limit"],
                             capped_games=sum(row["capped"] for row in rows),
                             decisive_games=sum(not row["capped"] for row in rows),
