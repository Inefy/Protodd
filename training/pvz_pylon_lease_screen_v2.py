"""Apply frozen Pylon lease gates to a mixed capped/decisive diagnostic run."""
...
def screen(plan_path, candidate_report_path):
    ...
    # The original cap-only reviewer rejected a genuine normal loss before the
    # frame limit. This amended read-only reviewer is pinned in its own report.
    from . import pvz_build_lease_mixed_review
    if candidate["review_source_sha256"] != sha256(pvz_build_lease_mixed_review.__file__):
        raise ValueError("amended reviewer differs")
    ...
    for game_id in sorted(original):
        ...
        if (a["seed"], a["map_hash"], a["map"], a["home"]) != \
            ...
            raise ValueError(f"seed, map or host mismatch: {game_id}")
        paired.append(dict(game_id=game_id, seed=a["seed"], map=a["map"], home=a["home"],
                           reference_frame=a["frame"], candidate_frame=b["frame"],
                           candidate_capped=b["capped"],
    ...
    passed = (len(paired) == plan["games"] and
              candidate["summary"]["capped_games"] == gates["normal_capped_pairs"] and
    ...
    return dict(schema="protodd-pvz-pylon-lease-screen-v2", plan_sha256=sha256(plan_path),
                ...
                reviewer_amendment="normal decisive game before frame cap",
